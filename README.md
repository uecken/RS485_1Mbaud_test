...
> **注**: 本検証は **ESP32 からみた通信成否の実測** (送受信フレーム一致率) であり、
> **オシロスコープで RS-485 A/B 線の波形を直接観測したものではない**。
> 「立上り時間」「スルーレート」等は IC データシートのカタログ値をもとにした推定・説明であり、
> 本プロジェクトで物理測定したのは **FER (フレームエラー率)** のみ。
...
上図は **IC データシートのカタログ立上り時間** を元にした**概念図**であり、本プロジェクトでオシロ観測した実波形ではない。
